(function() {
	...
	*/
	...
	//#endregion
	//#region src/core/background/tiktok-fetch-page-html.ts
	var AXIOS_CONFIG = {
		adapter: "fetch",
		timeout: 2e4,
		responseType: "text",
	...
	async function fetchTiktokVideoDetailPageHtml(url) {
		return (await axios.get(url, AXIOS_CONFIG)).data;
	}
	//#endregion
	...
	async function initBackground() {
		onRuntimeMessage({
			...
			"fetch tiktok page html": async ({ url }, { ok, fail }) => {
				try {
					return ok({ html: await fetchTiktokVideoDetailPageHtml(url) });
				} catch (e) {
